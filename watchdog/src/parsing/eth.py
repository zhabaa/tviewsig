...
class EthParsers:
    ...
    def parse_eth_signal(self, match, text, group):
        try:
            ...
            # emoji = match.group(4) USELESS EMOJI?
            action = match.group(5).upper()

            strength = level

            price_match = re.search(r"ETH/USDT-SPOT:\s*([\d,]+\.?\d*)", text)
            price = (
                float(price_match.group(1).replace(",", "")) if price_match else None
            )

            strength_match = re.search(r"Сила сигнала:.+?(\d+)\s*/\s*100", text)
            ai_strength = int(strength_match.group(1)) if strength_match else None

            conclusion_match = re.search(r"Вывод:\s*(.+?)(?:\n|$)", text)
